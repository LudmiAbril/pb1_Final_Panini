...
public class Qatar2022 {
	...
	private static void actualizardatosFigurita(Scanner teclado, Album actual) {
		...
		System.out.println("Selecci: " + aActualizar.getSeleccion());
		
		System.out.println("Ingrese el grupo al que pertenece la selecci");
	...
	private static void visualizarFiguritasActuales(Scanner teclado, Album actual) {
		Figurita[] actuales=actual.getFiguritasActuales();
		
		for(int i=0;i<actuales.length;i++) {
			if(actuales[i]!=null) {
				System.out.println(actuales[i].getCodigo());
			}
		}
